...
const ExpandableTask = ({ 
  ...
  const handleTitleSave = () => {
    ...
    onUpdateTask(index, { ...task, text: editedTitle });
  };

  const handleStopAndSave = () => {
    const hours = taskTimer / 3600;
    onTimeComplete(hours); // cite: 374, 405
  ...
  const containerStyle = isFullScreen ? {
    ...
    top: 0,
    left: 0,
    right: 0, // Ensure it anchors to both sides
    bottom: 0, // Ensure it anchors to bottom
    width: '100vw',
    height: '100vh',
    zIndex: 9999,
    background: '#000', // Solid black to hide background overflow
    ...
    boxSizing: 'border-box', // Crucial: ensures padding doesn't add to width
    minWidth: 0           // Allows the flex item to shrink
  };
  ...
  return (
    <div style={containerStyle}>
      ...
      {/* 1. PROGRESS BAR WITH PERCENTAGE (Expanded Only) */}
      {isFullScreen && (
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '16px', width: '100%', marginBottom: isFullScreen ? '24px' : '0'}}>
        <button
          onClick={() => {
            if (isChecked) {
              ...
              setSubtasks(updated);
              onUpdateTask(index, { ...task, subtasks: updated });
              onCheck(index);
            }
          }}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <button
            onClick={() => { onDeleteTask(index); if(isFullScreen) setIsFullScreen(false); }} // cite: 116, 423
